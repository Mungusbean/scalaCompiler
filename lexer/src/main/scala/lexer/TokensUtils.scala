...
def emitToken(ln: Int, cl: Int, c: Char)(using acc: String): LToken = {
    ...
    // Literals
    // Did not know parseInt could not do Hex and bianry by default
    // if (intRule.isResolved(curr_acc))    return IntTok(srcloc, curr_acc.toInt)
    if (intRule.isResolved(curr_acc)) {
      val value =
        if (curr_acc.startsWith("0b")) Integer.parseInt(curr_acc.drop(2), 2) // Handle binary rep
        else if (curr_acc.startsWith("0x")) Integer.parseInt(curr_acc.drop(2), 16) // Handle hex rep
        else curr_acc.toInt
      return IntTok(srcloc, value)
    }
